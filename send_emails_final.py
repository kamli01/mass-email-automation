"""
send_certificates_final.py
--------------------------
FINAL script -- uses Participants.xlsx
...
SENDER_PASSWORD = "ymer milx zhmi nsjq"        # Gmail App Password
SMTP_HOST       = "smtp.gmail.com"
SMTP_PORT       = 587

XLSX_FILE       = "Participants.xlsx"          # final file
...
def load_participants(xlsx_path: str):
    ...
    # Detect header row -- find column indices for name & email
    headers = [str(c.value).strip().lower() if c.value else "" for c in ws[1]]
    name_col = email_col = None
    for i, h in enumerate(headers):
        if "name" in h and "college" not in h and "company" not in h:
            if name_col is None:
                name_col = i
    ...
    print(f"[INFO] Using column {name_col} for name: '{headers[name_col]}'")
    print(f"[INFO] Using column {email_col} for email: '{headers[email_col]}'")
...
def main():
    ...
    print(f"\nFound {len(participants)} participant(s) and {len(cert_map)} certificate(s).\n")
    print(f"{'Name':<40} {'Email':<45} {'Certificate Found?'}")
    print("-" * 110)
    ...
    for name, email in participants:
        ...
        print(f"{name:<40} {email:<45} {status}")
